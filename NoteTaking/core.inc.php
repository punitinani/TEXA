...
function showLimitTen() {
    ...
    if ($num > 0) {
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            $date = $row['date'];
            echo html_entity_decode(" <li> <p><a href=\"show.inc.php?id=" . base64_encode($row['id']) . "&link=$i&title=$title\" class=\"list-group-item id=\"$i\" title=\" Created @ $date\" >" . $row['title'] . "     </a> </p></li>");
            $i++;
        }
    }
}
